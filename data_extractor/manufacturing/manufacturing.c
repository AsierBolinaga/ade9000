...
static bool manufacturing_write(void);
...
static bool manufaturing_compare_data(manufacturing_t* _written_manufacturing);
...
static bool manufacturing_write(void)
{
    ...
    if(ABSL_NVM_RV_OK == absl_nvm_erase(manufacturing_nvm, manufacturing_sector_index, 0, sizeof(manufacturing_t)))
    {
    	if(ABSL_NVM_RV_OK == absl_nvm_write(manufacturing_nvm, manufacturing_sector_index, 0,
    		     sizeof(manufacturing_t),(void*)& manufacturing_data))
    	{
			...
			if(true == manufaturing_compare_data(&written_manufacturing_data))
			{
				absl_debug_printf("\nWritten data:\n");
...
static bool manufaturing_compare_data(manufacturing_t* _written_manufacturing)
{
	if(strcmp(manufacturing_data.id_mumber, _written_manufacturing->id_mumber))
	{
		return false;
	}

	for(uint32_t mac_index = 0; mac_index < MAC_ADDRESS_SIZE; mac_index++)
	{
		if(manufacturing_data.mac_address[mac_index] != _written_manufacturing->mac_address[mac_index])
		{
			return false;
		}
	}

	if(strcmp(manufacturing_data.model, _written_manufacturing->model))
	{
		return false;
	}
	...
	for(uint32_t board_index = 0; board_index < BOARD_AMOUNT; board_index++)
	{
		if(strcmp(manufacturing_data.hw_version[board_index].board, _written_manufacturing->hw_version[board_index].board))
		{
			return false;
		}

		if(strcmp(manufacturing_data.hw_version[board_index].serial, _written_manufacturing->hw_version[board_index].serial))
		{
			return false;
		}

		if(strcmp(manufacturing_data.hw_version[board_index].manufacturer, _written_manufacturing->hw_version[board_index].manufacturer))
		{
			return false;
		}

		if(strcmp(manufacturing_data.hw_version[board_index].hw_version, _written_manufacturing->hw_version[board_index].hw_version))
		{
			return false;
		}
	}
