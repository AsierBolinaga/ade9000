	...
	if(NULL != _time_sync_config)
	{
		...
		if(ABSL_NTP_RV_OK == absl_ntp_init(&sync_ntp, sync_ntp_config))
		{
			if(ABSL_EVENT_RV_OK == absl_event_create(time_sync_config->time_sync_events) &&
			  (ABSL_QUEUE_RV_OK == absl_queue_create(_time_sync_config->watchdog_queue, sizeof(consistency_counter), 1)))
			{
...
void time_sync_task(void* arg)
{
	...
	while(1)
	{
		...
		if(TIMESYNC_WATCHDOG == (event_flags & TIMESYNC_WATCHDOG))
		{
			consistency_counter++;
			absl_queue_send(time_sync_config->watchdog_queue, (void*)&consistency_counter, 1);
		}
		...
		if(TIMESYNC_SYNC_MSG == (event_flags & TIMESYNC_SYNC_MSG))
		{
			...
			{
				if(true == time_sync_get_sync_period(&sync_period))
				{
				...
				{
					time_sync_notify_system_event(time_sync_config->event_info_array, TS_EVENTS_INCORRECT_POLL_PERIOD);
				}
			}
