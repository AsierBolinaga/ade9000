...
bool fw_update_task_initialize(fw_update_thread_config_t* _fw_update_task_config)
{
	...
	if(NULL != _fw_update_task_config)
	{
		...
		if((ABSL_EVENT_RV_OK == absl_event_create(fw_update_config->fw_update_event_group)) &&
		   ...
		   (ABSL_QUEUE_RV_OK == absl_queue_create(fw_update_config->watchdog_queue, sizeof(consistency_counter), 1)))
        {
...
void fw_update_task(void *arg)
{
	...
	while(1)
	{
        if(ABSL_EVENT_RV_OK == absl_event_wait(fw_update_config->fw_update_event_group, FW_UPDATE_EVENTS, &recieved_events))
		{
			...
			if(FW_UPDATE_WATCHDOG == (recieved_events & FW_UPDATE_WATCHDOG))
			{
				consistency_counter++;
				absl_queue_send(fw_update_config->watchdog_queue, (void*)&consistency_counter, 1);
			}
        }
        else
        {
...
static void fw_update_erase_new_fw_area(void)
{
	if(ABSL_NVM_RV_ERROR == absl_nvm_erase(&fw_update_nvm, slot_to_update_index, erase_offset, fw_update_config->erase_chunks))
	{
		...
		update_error_detected = true;
        absl_hardfault_handler(ERROR_FLASH_ERASE_FAILED);
	}
	else
	{
