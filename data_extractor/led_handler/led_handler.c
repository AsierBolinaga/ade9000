    ...
    if(NULL != _led_thread)
    {
		...
		if(NULL != (led_thread_config) && (NULL != led_thread_data))
		{
			...
			if((ABSL_GPIO_RV_OK == absl_gpio_init(&_led_thread->led_thread_data->status_led_user_gpio, status_led_user_config, ABSL_GPIO_NO_INT)) &&
			   (ABSL_EVENT_RV_OK == absl_event_create(led_thread_config->led_handler_event)) &&
			   (ABSL_QUEUE_RV_OK == absl_queue_create(led_thread_config->watchdog_queue, sizeof(led_thread_data->consistency_counter), 1)))
			{
				...
				if(ABSL_TIMER_RV_ERROR != absl_timer_create(&_led_thread->led_thread_data->led_toggle_timer, &led_handler_toggle_event,
												        led_thread_config->led_handler_event, toggle_time, false, true))
				{
					_led_thread->led_thread_data->consistency_counter = 0;
...
void led_handler_task(void* arg)
{
    ...
    while(1)
    {
        if(ABSL_EVENT_RV_OK == absl_event_wait(led_thread_config->led_handler_event, LED_HANDLER_EVENTS, &event_flags))
        {
        	if(LED_HANDLER_TOGGLE_EVENT == (event_flags & LED_HANDLER_TOGGLE_EVENT))
        	{
        	...
        	if(LED_HANDLER_WATCHDOG == (event_flags & LED_HANDLER_WATCHDOG))
        	{
        		led_thread_data->consistency_counter++;
        		absl_queue_send(led_thread_config->watchdog_queue, (void*)&led_thread_data->consistency_counter, 1);
        	}
        }
    }
}
