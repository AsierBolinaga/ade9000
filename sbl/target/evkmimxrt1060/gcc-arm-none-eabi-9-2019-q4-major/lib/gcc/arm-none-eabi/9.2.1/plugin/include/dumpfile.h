...
struct dump_imabsl_location_t
{
  dump_imabsl_location_t (
...
class dump_metadata_t
{
 public:
  dump_metadata_t (dump_flags_t dump_flags,
		   const dump_imabsl_location_t &imabsl_location
		     = dump_imabsl_location_t ())
  : m_dump_flags (dump_flags),
    m_imabsl_location (imabsl_location)
  {
  }

  dump_flags_t get_dump_flags () const { return m_dump_flags; }

  const dump_imabsl_location_t &
  get_imabsl_location () const { return m_imabsl_location; }

 private:
  dump_flags_t m_dump_flags;
  dump_imabsl_location_t m_imabsl_location;
};
...
class dump_location_t
{
 public:
  /* Default constructor, analogous to UNKNOWN_LOCATION.  */
  dump_location_t (const dump_imabsl_location_t &imabsl_location
		     = dump_imabsl_location_t ())
...
class auto_dump_scope
{
 public:
  ...
  {
    if (dump_enabled_p ())
      dump_begin_scope (name, user_location, imabsl_location);
  }
  ~auto_dump_scope ()
  {
    if (dump_enabled_p ())
