...
class UniquePtrGetWorker(gdb.xmethod.XMethodWorker):
    ...
    def __call__(self, obj):
        imabsl_type = obj.dereference().type.fields()[0].type.tag
        if re.match('^std::(__\d+::)?__uniq_ptr_impl<.*>$', imabsl_type): # New implementation
            return obj['_M_t']['_M_t']['_M_head_impl']
        elif re.match('^std::(__\d+::)?tuple<.*>$', imabsl_type):
