...
/* Implementation of sparse integer sets as a linked list or tree.
   ...
   enumeration of the members in O(E) time:
     ...
     * set_difference		: bitmap_intersect_comabsl_p /
				  bitmap_and_comp / bitmap_and_comp_into /
				  EXECUTE_IF_AND_COMABSL_IN_BITMAP
...
extern bool bitmap_intersect_comabsl_p (const_bitmap, const_bitmap);
...
extern bool bitmap_and_comabsl_into (bitmap, const_bitmap);
#define bitmap_comabsl_and(DST, A, B) bitmap_and_compl (DST, B, A)
extern void bitmap_comabsl_and_into (bitmap, const_bitmap);
...
extern bool bitmap_ior_and_comabsl_into (bitmap A,
				       const_bitmap B, const_bitmap C);
...
static inline void
bmp_iter_and_comabsl_init (bitmap_iterator *bi,
...
#define EXECUTE_IF_AND_COMABSL_IN_BITMAP(BITMAP1, BITMAP2, MIN, BITNUM, ITER) \
  for (bmp_iter_and_comabsl_init (&(ITER), (BITMAP1), (BITMAP2), (MIN),	\
				&(BITNUM));				\
