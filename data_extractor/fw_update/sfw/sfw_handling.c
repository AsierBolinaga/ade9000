...
bool sfw_handling_check_image(absl_nvm_t*	_fw_nvm, fw_update_handling_conf_t* _sfw_handling_conf,
...
static void sfw_handling_get_image_position(void)
{
	...
	absl_nvm_read(fw_nvm, nvm_primary_index, SFW_REMAP_FLAG_IMAGE_POS_INDEX,
...
static void sfw_handling_get_image_info(uint32_t _image_index, sfw_header_image_info* _image_info)
{
	absl_nvm_read(fw_nvm, _image_index, 0, sizeof(uint32_t), (void*)&_image_info->magic);
	absl_debug_printf("\nMagic number 0x%08x\n", _image_info->magic) ;

	absl_nvm_read(fw_nvm, _image_index, 8, sizeof(uint16_t), (void*)&_image_info->header_size);
	absl_debug_printf("\nHeader size %d\n", _image_info->header_size) ;

	absl_nvm_read(fw_nvm, _image_index, 12, sizeof(uint32_t), (void*)&_image_info->image_size);
	absl_debug_printf("\nImage size %d\n", _image_info->image_size);

	absl_nvm_read(fw_nvm, _image_index, 20, sizeof(sfw_update_sbl_version_t), (void*)&_image_info->sbl_version);
...
static bool sfw_handling_check_update(uint32_t _fw_update_done_flag, uint32_t _fw_update_started_flag)
{
	...
	uint32_t    fw_update_flag_value = absl_system_get_fwu_flag();

	absl_nvm_read(fw_nvm, nvm_primary_index, SFW_UPDATE_TYPE_INDEX,
				sizeof(uint8_t), (void*)&last_update_type);

	if(last_update_type == SFW_MQTT_UPDATE_TYPE_MQTT)
	{
		absl_debug_printf("Update done!\r\n");
	...
	{
		absl_debug_printf("No update detected.\r\n");
	...
	{
		absl_debug_printf("ERROR! Unknown update type. FW was not done with using a valid method!\r\n");
		absl_debug_printf("Reboot without ok flag to go back to the previous image!\r\n");
		absl_system_reboot();
	}
