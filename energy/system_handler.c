...
#define FAST_READ_TASK_STACK_SIZE 	1500

#define SLOW_READ_TASK_PRIORITY 	9
#define SLOW_READ_TASK_STACK_SIZE 	1024

#define FAST_STREAM_TASK_PRIORITY 	8
#define FAST_STREAM_TASK_STACK_SIZE 2048

#define SLOW_STREAM_TASK_PRIORITY 	6
#define SLOW_STREAM_TASK_STACK_SIZE 1500

#define CMD_TASK_PRIORITY 			5
#define CMD_TASK_STACK_SIZE 		3500
...
static waveform_data_t 	fast_vars_data;
...
absl_time_t system_sync_get_sync_period(void)
{
	...
	char* s_interval = strtok(sync_interval, "h");
	uint64_t u_interval;
	sscanf(s_interval, "%llu", &u_interval);

	period.seconds = u_interval * 3600;
	period.nseconds = 0;
	...
	if(ABSL_NVM_RV_OK == absl_nvm_init(&qspi_nvm, nvm_config))
	{
		if(ABSL_EVENT_RV_OK == absl_event_create(&system_events))
		{
			...
			(ABSL_QUEUE_RV_OK == absl_queue_create(&send_alets_queue, sizeof(alert_data_t), 5)))
			{
				...
				watchdog_initialize();
				system_hardfault_error_reset_check();
