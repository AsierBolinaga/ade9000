...
bool debug_shell_initialize(debug_shell_config_t* _debug_shell_config)
{
	...
	if(NULL != _debug_shell_config)
	{
		...
		pl_debug_shell_init(debug_shell_config->prompt);
...
void debug_shell_add_commads(debug_shell_cmd_t* _debug_shell_commads, uint32_t _cmd_amount)
{
	...
	for(uint8_t cmd_index = 0; cmd_index < _cmd_amount; cmd_index++)
	{
		...
		pl_debug_shell_add_cmd(_debug_shell_commads[cmd_index].command, _debug_shell_commads[cmd_index].descriptor);
	}
...
void debug_shell(void* arg)
{
	...
	{
		pl_debug_printf("Debug shell was not initialized!\n");
		pl_hardfault_handler(THREAD_NOT_INIT_ERROR);
	}

	pl_debug_enable_prompt();
	pl_debug_print_prompt();

	while(1)
	{
		while(1)
		{
			pl_debug_shell_wait_commad(argv);
