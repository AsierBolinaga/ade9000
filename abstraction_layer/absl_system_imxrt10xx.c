...
#if defined(ABSL_IMX_RT10XX) && defined(ABSL_OS_FREE_RTOS)
...
static absl_timer_t cpu_load_timer;

volatile float g_cpu_load_percent;
...
void absl_system_cpu_load_calc_cb(void* _arg)
{
    ABSL_UNUSED_ARG(_arg);

    uint32_t now_idle = ulTaskGetIdleRunTimeCounter();
    uint32_t total = absl_get_runtime_counter_value_freertos();

    uint32_t delta_idle = now_idle - last_idle;

    last_idle = now_idle;

	float idle_percent = ((float)delta_idle * 100.0f) / (float)total;
	g_cpu_load_percent = 100.0f - idle_percent;

	McuRTOS_RunTimeCounter = 0;
}

void absl_system_init_imxrt10xx(void)
{
    absl_time_t cpu_load_time;
    ...
    cpu_load_time.seconds = 1;
    cpu_load_time.nseconds = 0;
    absl_timer_create(&cpu_load_timer, &absl_system_cpu_load_calc_cb, NULL, cpu_load_time, true, true);
}

void vApplicationStackOverflowHook( TaskHandle_t xTask, char * pcTaskName )
{
	ABSL_UNUSED_ARG(xTask);

	absl_debug_printf("ERROR!! Stack overflow of thread %s!\n", pcTaskName);
	absl_hardfault_handler(STACK_OVERFLOW_FLAG);
}

void vApplicationMallocFailedHook(void)
{
	absl_debug_printf("ERROR!! malloc failed! Heap full or memory corruption\n");
	absl_hardfault_handler(MALLOC_FAILED_FLAG);
}

void absl_system_reboot_imxrt10xx(void)
{
	absl_debug_printf("system is rebooting...\r\n\r\n");
	absl_thread_sleep(500);
	NVIC_SystemReset();
}

void absl_system_configure_timer_for_runtime_stats_freertos( void )
{
	uint32_t gptFreq;
...
unsigned long absl_get_runtime_counter_value_freertos( void )
{
	return McuRTOS_RunTimeCounter;
}

float absl_system_supervisor_get_CPU_load_imxrt10xx(void)
{
	return g_cpu_load_percent;
}

float absl_system_supervisor_get_heap_usage_imxrt10xx(void)
{
...
absl_system_reset_t absl_system_get_reset_reason_imxrt10xx(void)
{
	...
	{
		reset_reason = ABSL_SYSTEM_POWERUP_RESET;
	...
	{
		reset_reason = ABSL_SYSTEM_SOFTWARE_RESET;
...
void absl_system_set_fwu_flag_imxrt10xx(uint32_t _flag)
{
	SNVS->LPGPR[LP_FLAGS_FWU_POS] = _flag;
}

uint32_t absl_system_get_fwu_flag_imxrt10xx(void)
{
	return SNVS->LPGPR[LP_FLAGS_FWU_POS];
}

void absl_system_get_fwu_clear_flag_imxrt10xx(void)
{
	SNVS->LPGPR[LP_FLAGS_FWU_POS] = 0;
}

void absl_system_set_hf_error_flag_imxrt10xx(uint32_t _flag)
{
	SNVS->LPGPR[LP_FLAGS_HF_ERRORS_POS] = _flag;
}

uint32_t absl_system_get_hf_error_flag_imxrt10xx(void)
{
...
uint32_t absl_system_get_hf_info_flag_imxrt10xx(void)
{
...
void absl_hardfault_handler_imxrt10xx(uint32_t _error_flag)
{
    PRINTF("HARDFAULT!\n");

    absl_system_set_hf_error_flag_imxrt10xx(_error_flag);
...
void absl_hardfault_mem_man_imxrt10xx(void)
{
	...
	if(mem_man_fault_reg && BFARVALID) /* if is set the value of the address where the fault occurred is stored in BFAR */
	{
		...
		absl_debug_printf("Bus fault in address: %d\n", SCB->BFAR);
	}
	else
	{
		SNVS->LPGPR[LP_FLAGS_HF_INFO] = 0;
	...
	absl_hardfault_handler(mem_man_fault);
}

void absl_hardfault_bus_imxrt10xx(void)
{
	...
	if(bus_fault_reg && MMARVALID) /* if is set the value of the address where the fault occurred is stored in MMFAR */
	{
		...
		absl_debug_printf("Mem management fault in address: %d\n", SCB->MMFAR);
	}
	else
	{
		SNVS->LPGPR[LP_FLAGS_HF_INFO] = 0;
	...
	absl_hardfault_handler(bus_fault);
}

void absl_hardfault_usage_imxrt10xx(void)
{
	...
	absl_hardfault_handler(usage_fault);
}
