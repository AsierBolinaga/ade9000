...
class UniquePointerPrinter:
    ...
    def __init__ (self, typename, val):
        self.val = val
        imabsl_type = val.type.fields()[0].type.tag
        if is_specialization_of(imabsl_type, '__uniq_ptr_impl'): # New implementation
            self.pointer = val['_M_t']['_M_t']['_M_head_impl']
        elif is_specialization_of(imabsl_type, 'tuple'):
            self.pointer = val['_M_t']['_M_head_impl']
        else:
            raise ValueError("Unsupported implementation for unique_ptr: %s" % imabsl_type)
