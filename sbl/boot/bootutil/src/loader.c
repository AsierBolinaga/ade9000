...
#ifdef CONFIG_BOOT_SIGNATURE  //Tim added
...
#if !defined(__BOOTSIM__)
        BOOT_LOG_ERR("Signature verification failed, Image in the %s slot is not valid!\r\n",
...
//    fih_int fih_rc = FIH_FAILURE;
    ...
    if (BOOT_IS_UPGRADE(swap_type)) {
         ...
         */
#ifdef CONFIG_BOOT_SIGNATURE  //Tim added
//        FIH_CALL(boot_validate_slot, fih_rc, state, BOOT_SECONDARY_SLOT, bs);  //comment to Meet the link address requirements of security
//        if (fih_not_eq(fih_rc, FIH_SUCCESS)) {
...
#define REMAP_MAGIC_GOOD     1
#define REMAP_MAGIC_BAD      2
#define REMAP_MAGIC_UNSET    3

#define REMAP_FLAG_SET       1
#define REMAP_FLAG_BAD       2
#define REMAP_FLAG_UNSET     3
#define REMAP_FLAG_REVERT    4

/** Attempt to boot the contents of the primary slot. */
#define BOOT_REMAP_TYPE_NONE     1
#define BOOT_REMAP_TYPE_TEST     2
#define BOOT_REMAP_TYPE_PERM     3
#define BOOT_REMAP_TYPE_REVERT   4

#define FLAG_ERASE_SIZE 0x1000

const uint32_t REMAP_MAX_ALIGN = BOOT_MAX_ALIGN;
/* This is not actually used by mcuboot's code but can be used by apps
 * when attempting to read/write a remap flag.
 */
...
static struct remap_state remap_status;

extern flash_ops_s mcuboot_flash;

static uint32_t
remap_magic_off(void)
{
    assert(offsetof(struct remap_trailer, magic) == 16);
    return BOOT_FLASH_ACT_APP - REMAP_MAGIC_SZ;
}

static int
remap_magic_decode(const uint32_t *magic)
{
    if (memcmp(magic, boot_remap_magic, REMAP_MAGIC_SZ) == 0) {
        return REMAP_MAGIC_GOOD;
    }
    return REMAP_MAGIC_BAD;
}
...
static uint32_t
remap_image_ok_off(void)
{
    assert(offsetof(struct remap_trailer, image_ok) == 8);
    return BOOT_FLASH_ACT_APP - REMAP_MAGIC_SZ - REMAP_MAX_ALIGN;
}

int metadata_area_read_is_empty(uint32_t off, void *dst, uint32_t len)
{
	uint8_t i;
    uint8_t *u8dst;

    mcuboot_flash.flash_read(off, dst, len);
...
/**
 * Reads the remap trailer from the slot.
 */
int
boot_read_remap_state(struct remap_state *state)
{
    uint32_t magic[REMAP_MAGIC_ARR_SZ];
    uint32_t off;
    int rc;
    
    off = remap_magic_off();
    rc = metadata_area_read_is_empty(off, magic, REMAP_MAGIC_SZ);
    if (rc < 0) {
        return BOOT_EFLASH;
    }
    if (rc == 1) {
        state->magic = REMAP_FLAG_UNSET;
    } else {
        state->magic = remap_magic_decode(magic);
    }

    off = remap_image_position_off();
    rc = metadata_area_read_is_empty(off, &state->image_position,
                sizeof state->image_position);
        if (rc < 0) {
            return BOOT_EFLASH;
        }

    off = remap_image_ok_off();
    rc = metadata_area_read_is_empty(off, &state->image_ok,
                                  sizeof state->image_ok);
    if (rc < 0) {
        return BOOT_EFLASH;
    }
    if (rc == 1) {
        state->image_ok = REMAP_FLAG_UNSET;
    } else {
...
/* Return the remap type */
int
boot_read_remap_type(void)
{
    if ((remap_status.magic == REMAP_MAGIC_GOOD) &&
        (remap_status.image_ok == REMAP_FLAG_UNSET))
    {   
        return BOOT_REMAP_TYPE_TEST;
    }
    else if ((remap_status.magic == REMAP_MAGIC_GOOD) &&
             (remap_status.image_ok == REMAP_FLAG_SET))
    {
        return BOOT_REMAP_TYPE_PERM;
    }
    else if ((remap_status.magic == REMAP_MAGIC_GOOD) &&
             (remap_status.image_ok == REMAP_FLAG_REVERT))
    {
        return BOOT_REMAP_TYPE_REVERT;
    }
    else
    {
        return BOOT_REMAP_TYPE_NONE;
    }
}

static int
boot_set_remap_flag(uint8_t image_position)
{
    int rc;
    uint8_t remap_trailer[32] = {0};
 
    mcuboot_flash.flash_read(BOOT_FLASH_ACT_APP- REMAP_MAGIC_SZ*2, remap_trailer, REMAP_MAGIC_SZ*2);
    mcuboot_flash.flash_erase(BOOT_FLASH_ACT_APP-FLAG_ERASE_SIZE-BOOT_FLASH_BASE, FLAG_ERASE_SIZE);
    
    remap_trailer[0] = image_position;
    remap_trailer[REMAP_MAX_ALIGN] = REMAP_FLAG_REVERT;

    rc = mcuboot_flash.flash_write(BOOT_FLASH_ACT_APP- REMAP_MAGIC_SZ*2-BOOT_FLASH_BASE, remap_trailer, REMAP_MAGIC_SZ*2);;
    if (rc != 0) {
        return BOOT_EFLASH;
    }
...
boot_clear_remap_flag(void)
{
    ...
    memset((void *)&remap_trailer[1], 0xff, REMAP_TRAILER_SIZE - 1);
...
/**
 * Prepares the booting process.  This function verify the new image
 *  and tell you which address to boot to.
 *
 * @param rsp                   On success, indicates how booting should occur.
 *
 * @return                      0 on success; nonzero on failure.
...
int boot_remap_go(struct boot_rsp *rsp)
{
    ...
    /* Attempt to read the remap state. */
    rc = boot_read_remap_state(&remap_status);
    if (rc != 0) {
        goto out;
    }
    if (remap_status.image_position == FLASH_AREA_IMAGE_PRIMARY(0))
    {
        SBL_EnableRemap(BOOT_FLASH_ACT_APP, BOOT_FLASH_ACT_APP+FLASH_AREA_IMAGE_1_SIZE, FLASH_AREA_IMAGE_1_SIZE);
    }
    else if (remap_status.image_position == FLASH_AREA_IMAGE_SECONDARY(0)) 
    {
    }
    else
    {
        boot_set_remap_flag(FLASH_AREA_IMAGE_PRIMARY(0));
        boot_clear_remap_flag();
        remap_status.image_position = FLASH_AREA_IMAGE_PRIMARY(0);
    }
    /* The array of slot sectors are defined here (as opposed to file scope) so
     * that they don't get allocated for non-boot-loader apps.  This is
     * necessary because the gcc option "-fdata-sections" doesn't seem to have
    ...
    boot_data.imgs[0][BOOT_PRIMARY_SLOT].sectors = primary_slot_sectors;
    boot_data.imgs[0][BOOT_SECONDARY_SLOT].sectors = secondary_slot_sectors;
    ...
    /* Attempt to read an image header from each slot. */
    rc = boot_read_image_headers(&boot_data, false, NULL);
    if (rc != 0) {
        goto out;
    }
    /* From the remap state, judege the remap type this time */
    remap_type = boot_read_remap_type();

    /* Fix the issue that dbg_console may crash */
    __DSB();
    
    BOOT_LOG_INF("Remap type: %s\r\n",
                     remap_type == BOOT_REMAP_TYPE_TEST   ? "test"   :
                     remap_type == BOOT_REMAP_TYPE_PERM   ? "perm"   :
                     remap_type == BOOT_REMAP_TYPE_REVERT ? "revert" :
                     remap_type == BOOT_REMAP_TYPE_NONE   ? "none"   :
                     "BUG; can't happen\r\n");
    switch (remap_type) {
    case BOOT_REMAP_TYPE_TEST:          /* fallthrough */
    case BOOT_REMAP_TYPE_PERM:          /* fallthrough */
    case BOOT_REMAP_TYPE_REVERT:
        if (remap_status.image_position == FLASH_AREA_IMAGE_PRIMARY(0))
        {
            BOOT_LOG_INF("Image postion: %d\r\n", remap_status.image_position);
            ...
            if (fih_not_eq(fih_rc, FIH_SUCCESS)) {
                ...
                if (remap_type == BOOT_REMAP_TYPE_TEST) {
                    ...
                    /* Update new image failed, original image is in slot2, restore the key block, 
                     * reset and try to run original image
                     */
                    update_key_context(FLASH_AREA_IMAGE_2_OFFSET + KEY_CONTEXT_OFFSET_IN_APP);
                }
#endif
        ...
        if(remap_type == BOOT_REMAP_TYPE_REVERT || remap_type == BOOT_REMAP_TYPE_PERM)
        {
            boot_clear_remap_flag();
        }
        BOOT_LOG_INF("The image now in %s slot\r\n", remap_status.image_position == FLASH_AREA_IMAGE_PRIMARY(0) ? "SECONDARY_SLOT" :
                         remap_status.image_position == FLASH_AREA_IMAGE_SECONDARY(0) ? "PRIMARY_SLOT" :
                         "Can't happen");
        break;
    case BOOT_REMAP_TYPE_NONE:
        if (remap_status.image_position == FLASH_AREA_IMAGE_PRIMARY(0))
        {
            SBL_DisableRemap();
            __DSB();
        ...
        {
            SBL_EnableRemap(BOOT_FLASH_ACT_APP, BOOT_FLASH_CAND_APP, BOOT_FLASH_CAND_APP - BOOT_FLASH_ACT_APP);
            __DSB();
#ifdef CONFIG_BOOT_SIGNATURE
            /* Attempt to read an image header from each slot. */
            rc = boot_read_image_headers(&boot_data, false, NULL);
        ...
        BOOT_LOG_INF("The image now in %s slot\r\n", remap_status.image_position == FLASH_AREA_IMAGE_PRIMARY(0) ? "PRIMARY_SLOT" :
                         remap_status.image_position == FLASH_AREA_IMAGE_SECONDARY(0) ? "SECONDARY_SLOT" :
                         "Can't happen");
        break;   
    }
     ...
     * the magic number on the image is OK.
     */
    if (boot_data.imgs[0][BOOT_PRIMARY_SLOT].hdr.ih_magic != IMAGE_MAGIC) {
        BOOT_LOG_ERR("bad image magic 0x%lx\r\n",
                (unsigned long)boot_data.imgs[0][BOOT_PRIMARY_SLOT].hdr.ih_magic);
        rc = BOOT_EBADIMAGE;
        goto out;
    }
    ...
    rsp->br_flash_dev_id = boot_data.imgs[0][BOOT_PRIMARY_SLOT].area->fa_device_id;
