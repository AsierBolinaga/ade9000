...
static absl_phy_rv_t absl_phy_set_default_ip_addresses(absl_phy_t* _absl_phy, ethernetif_config_t* _fsl_enet_config)
{
	...
	IP4_ADDR(&_absl_phy->netif_subnet_mask, _absl_phy->phy_config->default_subnet_mask[0],
									   _absl_phy->phy_config->default_subnet_mask[1],
									   _absl_phy->phy_config->default_subnet_mask[2],
									   _absl_phy->phy_config->default_subnet_mask[3]);
	...
	err = netifapi_netif_add(&_absl_phy->netif, &_absl_phy->netif_local_addr, &_absl_phy->netif_subnet_mask, &_absl_phy->netif_gw_addr,
	...
	while (netif_is_up(&_absl_phy->netif) && !process_done)
	{
		...
		else if (dhcp_last_state != dhcp->state)
		{
			...
			if (dhcp_last_state == DHCP_STATE_BOUND)
			{
				...
				_absl_phy->netif_subnet_mask = dhcp->offered_sn_mask;
...
static void absl_phy_print_ip_addresses(absl_phy_t* _absl_phy)
{
	...
	absl_debug_printf(" IPv4 Subnet mask : %s\r\n", ipaddr_ntoa(&_absl_phy->netif_subnet_mask));
