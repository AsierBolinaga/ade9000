...
status_t mflash_drv_if_erase(uint32_t start, uint32_t length)
{
    status_t status = kStatus_Success;

    if (!mflash_drv_is_sector_aligned(start) || !mflash_drv_is_sector_aligned(length))
        return kStatus_InvalidArgument;

    while (length)
    {
        status = (status_t)mflash_drv_sector_erase(start);
        if (status != kStatus_Success)
            break;

        start += MFLASH_SECTOR_SIZE;
        length -= MFLASH_SECTOR_SIZE;
    }

    return status;
}

status_t mflash_drv_if_program(uint32_t start, uint8_t *src, uint32_t length)
{
    status_t status = kStatus_Success;
    uint32_t chunk_len;

    if (!mflash_drv_is_page_aligned(start))
        return kStatus_InvalidArgument;

    while (length)
    {
        if (length < MFLASH_PAGE_SIZE)
        {
            uint32_t buf[MFLASH_PAGE_SIZE / 4];
            memset(buf, 0xff, sizeof(buf));
            memcpy(buf, src, length);
            chunk_len = length;
            status    = (status_t)mflash_drv_page_program(start, (void *)buf);
        }
        else
        {
            chunk_len = MFLASH_PAGE_SIZE;
            status    = (status_t)mflash_drv_page_program(start, (void *)src);
        }
...
flash_driver_interface_t mflash_drv_if = {.erase = mflash_drv_if_erase, .program = mflash_drv_if_program};

flash_driver_interface_t *flash_driver_if = &mflash_drv_if;

#else // BL_USE_MFLASH

flash_driver_interface_t *flash_driver_if = NULL;

#endif // BL_USE_MFLASH

/* Retrieves API version of the reliable update interface while checking validity of the interface pointer. Zero result
 * indicates no API present. */
#define BL_API_VERSION                                                                       \
    ((*reliable_update_if_ptr != NULL) && (*reliable_update_if_ptr != (void *)0xFFFFFFFF) && \
             ((*reliable_update_if_ptr)->magic_num == RELIABLE_UPDATE_INTERFACE_MAGIC) ?     \
         ((*reliable_update_if_ptr)->version) :                                              \
         0)

/* Major number of the API version should be 1 */
#define BL_API_VERSION_CHECK ((BL_API_VERSION & 0xFFFFFF00) >= 0x00010100)

static reliable_update_interface_t **reliable_update_if_ptr =
    (reliable_update_interface_t **)BL_RELIABLE_UPDATE_IF_ADDR;
...
uint32_t bl_crc32(const uint8_t *data, uint32_t size)
{
    crc32_data_t crc_data;
    uint32_t checksum;

    crc32_init(&crc_data);
    crc32_update(&crc_data, data, size);
    crc32_finalize(&crc_data, &checksum);
...
{
    boot_image_header_t *bih_ptr;

    /* Check parameters */
    if ((data == NULL) || (size < sizeof(boot_image_header_t)))
    {
        return -1;
    }

    bih_ptr = (boot_image_header_t *)data;

    /* Check image tag */
    if (bih_ptr->tag != IMG_HDR_TAG)
    {
        return -1;
    }

    /* Check size of the image */
    if (bih_ptr->header_size + bih_ptr->image_size != size)
    {
        return -1;
    }

    /* Verify checksum/signature for supported algorithms */
    if (bih_ptr->algorithm == IMG_CHK_ALG_CRC32)
    {
        uint32_t crc32 = bl_crc32(data + bih_ptr->header_size, bih_ptr->image_size);
        if (crc32 == bih_ptr->checksum[0])
        {
            /* Checksum ok */
            return 1;
        }
...
/* Returns version of bootloader reliable update interface API, if any, otherwise zero (no API present) */
uint32_t bl_api_version(void)
{
    return BL_API_VERSION;
}

void bl_set_flash_driver_if(flash_driver_interface_t *drvif)
{
    flash_driver_if = drvif;
}

status_t bl_get_update_partition_info(partition_t *partition)
{
    if (!BL_API_VERSION_CHECK)
    {
        return kStatus_Fail;
    }

	return (*reliable_update_if_ptr)->get_update_partition_info(partition);
}
...
status_t bl_update_image_state(uint32_t state)
{
    ...
    if (flash_driver_if != NULL)
    {
        status_t status;
        uint32_t primask;
        primask = DisableGlobalIRQ(); /* Workaround for update_image_state_user_api() not re-enabling interrupts as it
                                         should */
        status = (*reliable_update_if_ptr)->update_image_state_user_api(state, flash_driver_if);
        EnableGlobalIRQ(primask);
        return status;
    }

    return (*reliable_update_if_ptr)->update_image_state(state);
}
...
status_t bl_get_image_state(uint32_t *state)
{
    ...
    return (*reliable_update_if_ptr)->get_image_state(state);
}

status_t bl_update_partition_table(partition_t *partition, uint32_t entries)
{
    if (!BL_API_VERSION_CHECK)
    {
        return kStatus_Fail;
    ...
    return (*reliable_update_if_ptr)->update_partition_table_user_api(partition, entries, flash_driver_if);
}
