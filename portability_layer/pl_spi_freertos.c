...
static bool pl_spi_init_master_freertos(pl_spi_t * _spi);

static bool pl_spi_dmux_init_freertos(pl_spi_config_t * _spi_config);
static pl_spi_rv_t pl_spi_transfer_master_freertos(pl_spi_t * _spi, uint32_t _length, uint64_t* _tx_timestamp);
...
void LPSPI_MasterUserCallback(LPSPI_Type *base, lpspi_master_edma_handle_t *handle, status_t status, void *userData)
{
	...
	PL_UNUSED_ARG(base);
	PL_UNUSED_ARG(handle);

    if (status == kStatus_Success)
    {
    	spi->last_tx_ts = pl_time_to_us(pl_time_gettime());
 ...
 * 					FALSE:	init NOK
 */
pl_spi_rv_t pl_spi_init_freertos(pl_spi_t * _spi, pl_spi_config_t* _spi_config, uint8_t* tx_buff, uint8_t* rx_buff)
{
	...
	if(NULL != _spi_config)
	{
		...
		{
			if(pl_spi_init_master_freertos(_spi))
			{
				pl_mutex_create(&_spi->spi_mutex);
				pl_event_create(&_spi->transmision_event);
...
pl_spi_rv_t pl_spi_transfer_freertos(pl_spi_t * _spi, uint32_t _length)
{
	pl_spi_rv_t pl_spi_rv = PL_SPI_RV_ERROR;

	pl_mutex_take(&_spi->spi_mutex);
	if(PL_SPI_MODE_MASTER == _spi->spi_config->spi_mode)
	{
		pl_spi_rv = pl_spi_transfer_master_freertos(_spi, _length, NULL);
	}
	else
	{
...
pl_spi_rv_t pl_spi_ts_transfer_freertos(pl_spi_t * _spi, uint32_t _length, uint64_t* _tx_timestamp)
{
	pl_spi_rv_t pl_spi_rv = PL_SPI_RV_ERROR;
	...
	if(PL_SPI_MODE_MASTER == _spi->spi_config->spi_mode)
	{
		pl_spi_rv = pl_spi_transfer_master_freertos(_spi, _length, _tx_timestamp);
	}
	else
	{
		/* Todo - slave transmit has to be implemented */
...
void pl_spi_reset(pl_spi_t * _spi)
{
	...
	pl_spi_init_master_freertos( _spi);
}

uint64_t pl_spi_get_last_transfer_ts_freertos(pl_spi_t* _spi)
{
...
static pl_spi_rv_t pl_spi_transfer_master_freertos(pl_spi_t * _spi, uint32_t _length, uint64_t* _tx_timestamp)
{
	...
	if (status == kStatus_Success)
	{
		if(PL_EVENT_RV_OK == pl_event_timed_wait(&_spi->transmision_event, TRANSMISSION_COMPLETED,
...
static bool pl_spi_init_master_freertos(pl_spi_t * _spi)
{
	...
	if(true == pl_spi_dmux_init_freertos(_spi->spi_config))
	{
		/* EDMA init*/
		/*
