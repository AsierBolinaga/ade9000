...
static void absl_mqtt_clear_buff_vars(absl_mqtt_t * _mqtt);
...
absl_mqtt_rv_t absl_mqtt_init_freertos(absl_mqtt_t * _mqtt, absl_mqtt_config_t* _mqtt_config)
{
	...
	if(NULL!= _mqtt_config)
	{
		...
		if(ABSL_PHY_RV_OK == absl_phy_init(&_mqtt->mqtt_phy, _mqtt_config->mqtt_phy))
		{
			...
			absl_mqtt_clear_buff_vars(_mqtt);
...
absl_mqtt_rv_t absl_mqtt_connect_freertos(absl_mqtt_t * _mqtt, char* _client_id, absl_mqtt_topics_t _last_will, char* _last_will_msg)
{
	...
	if(false == _mqtt->mqtt_connected)
	{
		if(0 != *_client_id)
		{
			...
			if(ERR_OK == err)
			{
				if(ABSL_EVENT_RV_NO_EVENT != absl_event_timed_wait(&_mqtt->event_group, ABSL_MQTT_CONNECTED | ABSL_MQTT_TIMEOUT | ABSL_MQTT_DISCONNECTED, &events, 500))
				{
...
absl_mqtt_rv_t absl_mqtt_publish_freertos(absl_mqtt_t * _mqtt, char* _topic, char* _tx_buff, uint16_t _payload_length, uint8_t _qos, bool _retain)
{
	...
	if(true == _mqtt->mqtt_connected)
	{
		...
		if(ABSL_EVENT_RV_NO_EVENT !=  absl_event_timed_wait(&_mqtt->event_group, ABSL_MQTT_MESSAGE_SEND | ABSL_MQTT_TIMEOUT | ABSL_MQTT_ERROR, &events, 5000))
		{
...
{
	return  _mqtt->mqtt_buff[_mqtt->msg_index_out].topic;
}
...
{
	if(_mqtt->msg_count != 0)
	{
		mqtt_msg_t *msg = &_mqtt->mqtt_buff[_mqtt->msg_index_out];

		*_topic_length = msg->topic_len;
		memcpy(_topic, msg->topic, msg->topic_len);

		*_msg_length = msg->payload_len;
		memcpy(_rx_msg, msg->payload, msg->payload_len);

		memset(msg, 0, sizeof(mqtt_msg_t));

		_mqtt->msg_index_out = ABSL_INC_INDEX(_mqtt->msg_index_out, ABSL_MQTT_MSG_MAX_AMOUNT);

		_mqtt->msg_count--;
	}

	return _mqtt->msg_count;
}
...
uint32_t absl_mqtt_get_frame_freertos(absl_mqtt_t * _mqtt, char* _rx_msg)
{
	...
	frame_length = msg->payload_len;
	memcpy(_rx_msg, msg->payload, frame_length);
...
{
	return _mqtt->mqtt_buff[_mqtt->msg_index_out].topic_len;
}
...
static void absl_mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status)
{
	...
	switch (status)
	{
		case MQTT_CONNECT_ACCEPTED:
			absl_mqtt_clear_buff_vars(mqtt);
...
static void absl_mqtt_incoming_publish_cb(void *arg, const char *topic, u32_t tot_len)
{
	...
	if((mqtt->msg_count != 0) && (mqtt->msg_index_out == mqtt->msg_index_topic))
	{
	...
	{
		mqtt_msg_t *msg = &mqtt->mqtt_buff[mqtt->msg_index_topic];

		msg->topic_len = strlen(topic);
		memcpy(msg->topic, topic, msg->topic_len);
		msg->topic_obtained = true;

		mqtt->msg_index_topic = ABSL_INC_INDEX(mqtt->msg_index_topic, ABSL_MQTT_MSG_MAX_AMOUNT);
...
static void absl_mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
	...
	mqtt_msg_t *msg = &mqtt->mqtt_buff[mqtt->msg_index_in];

	if(true == msg->topic_obtained)
	{
		if((msg->payload_len + len) >= ABSL_MQTT_PAYLOAD_MAX_LEN)
		{
			absl_event_set_fromISR_freertos(&mqtt->event_group, ABSL_MQTT_SBUFFER_FULL);
			if(true == mqtt->config_event)
			{
		...
		else
		{
			...
			{
				mqtt->msg_index_in = ABSL_INC_INDEX(mqtt->msg_index_in, ABSL_MQTT_MSG_MAX_AMOUNT);
				mqtt->msg_count++;
...
static void absl_mqtt_clear_buff_vars(absl_mqtt_t * _mqtt)
{
	...
	{
		memset(&_mqtt->mqtt_buff[index], 0, sizeof(mqtt_msg_t));
	}

	_mqtt->msg_index_in = 0;
	_mqtt->msg_index_out = 0;
	_mqtt->msg_index_topic = 0;
	_mqtt->msg_count = 0;
}
