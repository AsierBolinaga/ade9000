...
bool json_get_energy_sensor_config(char* _config_data_buff, void* _config_data)
{
	...
	if(cjson_rx != NULL)
	{
		cJSON* cjson_tc_prim = NULL;
		cJSON* cjson_tc_sec = NULL;
		cJSON* cjson_freq = NULL;
		cJSON* cjson_hp_filter = NULL;
		cJSON* cjson_hw_config = NULL;

		cjson_tc_prim = cJSON_GetObjectItem(cjson_rx, "tc_primary_side");
		cjson_tc_sec = cJSON_GetObjectItem(cjson_rx, "tc_secondary_side");
		cjson_freq = cJSON_GetObjectItem(cjson_rx, "fundamental_frequency");
		cjson_hp_filter = cJSON_GetObjectItem(cjson_rx, "high_pass_filter_disable");
		memset(sensor_config->hw_config, 0, 50);
		cjson_hw_config = cJSON_GetObjectItem(cjson_rx, "hw_config");

		/* check if mandatory values missing */
		if((cjson_tc_prim != NULL) && (cjson_tc_sec != NULL) && (cjson_freq != NULL) && (cjson_hp_filter != NULL) && (cjson_hw_config != NULL))
		{
			cJSON* cjson_variable = NULL;

			sensor_config->tc_primary_side = cjson_tc_prim->valueint;
			sensor_config->tc_secondary_side = cjson_tc_sec->valueint;
			sensor_config->fundamental_frequency = cjson_freq->valueint;

			if(cJSON_IsTrue(cjson_hp_filter))
			{
			...
			memcpy(sensor_config->hw_config, cjson_hw_config->valuestring, strlen(cjson_hw_config->valuestring));
			...
			{
				/* Default values */
				sensor_config->OI_threshold = 	 OI_THREShOLD_DEFAULT;
				sensor_config->DIP_threshold = 	 DIP_THREShOLD_DEFAULT;
				sensor_config->SWELL_threshold = SWELL_THREShOLD_DEFAULT;
				/* TODO = Alert */
			}
			...
			if(cjson_variable != NULL)
			{
				...
				if((cjson_a != NULL) && cJSON_IsTrue(cjson_a))
				{
				...
				if((cjson_b != NULL) && (cJSON_IsTrue(cjson_b)))
				{
				...
				if((cjson_c != NULL) && cJSON_IsTrue(cjson_c))
				{
			...
			cjson_variable = cJSON_GetObjectItem(cjson_rx, "gain");
			if(cjson_variable != NULL)
			{
				cJSON* cjson_gain = NULL;

				cjson_gain = cJSON_GetObjectItem(cjson_variable, "ia");
				if(cjson_gain != NULL)
				{
					sensor_config->ia_gain = cjson_gain->valuedouble;
				}
				else
				{
					sensor_config->ia_gain = 1;
				}


				cjson_gain = cJSON_GetObjectItem(cjson_variable, "ib");
				if(cjson_gain != NULL)
				{
					sensor_config->ib_gain = cjson_gain->valuedouble;
				}
				else
				{
					sensor_config->ib_gain = 1;
				}


				cjson_gain = cJSON_GetObjectItem(cjson_variable, "ic");
				if(cjson_gain != NULL)
				{
					sensor_config->ic_gain = cjson_gain->valuedouble;
				}
				else
				{
					sensor_config->ic_gain = 1;
				}

				cjson_gain = cJSON_GetObjectItem(cjson_variable, "va");
				if(cjson_gain != NULL)
				{
					sensor_config->va_gain = cjson_gain->valuedouble;
				}
				else
				{
					sensor_config->va_gain = 1;
				}

				cjson_gain = cJSON_GetObjectItem(cjson_variable, "vb");
				if(cjson_gain != NULL)
				{
					sensor_config->vb_gain = cjson_gain->valuedouble;
				}
				else
				{
					sensor_config->vb_gain = 1;
				}

				cjson_gain = cJSON_GetObjectItem(cjson_variable, "vc");
				if(cjson_gain != NULL)
				{
					sensor_config->vc_gain = cjson_gain->valuedouble;
				}
				else
				{
					sensor_config->vc_gain = 1;
				}
			}
			else
			{
				/* TODO = Alert */
				sensor_config->ia_gain = GAIN_DEFAULT_VALUE;
				sensor_config->ib_gain = GAIN_DEFAULT_VALUE;
				sensor_config->ic_gain = GAIN_DEFAULT_VALUE;
				sensor_config->va_gain = GAIN_DEFAULT_VALUE;
				sensor_config->vb_gain = GAIN_DEFAULT_VALUE;
				sensor_config->vc_gain = GAIN_DEFAULT_VALUE;
			}
				...
				for(uint32_t adc_index = 0; adc_index < ADC_READ_NUM; adc_index++)
				{
					...
					if(cjson_adc != NULL)
					{
						...
						else
						{
							...
							sensor_config->adc_redirect[adc_index] = (adc_redirect_t)ADC_DEFAULT_VALUE;
						}
					}
					else
					{
