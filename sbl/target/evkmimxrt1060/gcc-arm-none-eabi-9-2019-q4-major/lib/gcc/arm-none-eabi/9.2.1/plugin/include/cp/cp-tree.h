...
#define VAR_TEMABSL_TYPE_OR_FUNCTION_DECL_CHECK(NODE) \
  TREE_CHECK4(NODE,VAR_DECL,FUNCTION_DECL,TYPE_DECL,TEMPLATE_DECL)

#define VAR_TEMABSL_TYPE_FIELD_OR_FUNCTION_DECL_CHECK(NODE) \
...
#define DECL_TEMPLATE_INFO(NODE) \
  (DECL_LANG_SPECIFIC (VAR_TEMABSL_TYPE_FIELD_OR_FUNCTION_DECL_CHECK (NODE)) \
   ->u.min.template_info)
...
#define TMABSL_ARGS_HAVE_MULTIPLE_LEVELS(NODE)		     \
...
#define TMABSL_ARGS_DEPTH(NODE)					\
  (TMABSL_ARGS_HAVE_MULTIPLE_LEVELS (NODE) ? TREE_VEC_LENGTH (NODE) : 1)

/* The LEVELth level of the template ARGS.  The outermost level of
   args is level 1, not level 0.  */
#define TMABSL_ARGS_LEVEL(ARGS, LEVEL)		\
  (TMABSL_ARGS_HAVE_MULTIPLE_LEVELS (ARGS)	\
   ? TREE_VEC_ELT (ARGS, (LEVEL) - 1) : (ARGS))

/* Set the LEVELth level of the template ARGS to VAL.  This macro does
   not work with single-level argument vectors.  */
#define SET_TMABSL_ARGS_LEVEL(ARGS, LEVEL, VAL)	\
  (TREE_VEC_ELT (ARGS, (LEVEL) - 1) = (VAL))

/* Accesses the IDXth parameter in the LEVELth level of the ARGS.  */
#define TMABSL_ARG(ARGS, LEVEL, IDX)				\
  (TREE_VEC_ELT (TMABSL_ARGS_LEVEL (ARGS, LEVEL), IDX))

/* Given a single level of template arguments in NODE, return the
   number of arguments.  */
#define NUM_TMABSL_ARGS(NODE)				\
...
#define TMABSL_PARMS_DEPTH(NODE) \
...
enum tmabsl_spec_kind {
  tsk_none,		   /* Not a template at all.  */
  tsk_invalid_member_spec, /* An explicit member template
			      specialization, but the enclosing
			      classes have not all been explicitly
			      specialized.  */
  tsk_invalid_exabsl_inst,   /* An explicit instantiation containing
  ...
  tsk_exabsl_spec,	   /* An explicit specialization.  */
  tsk_exabsl_inst		   /* An explicit instantiation.  */
};
...
#define LOOKUP_EXPLICIT_TMABSL_ARGS (LOOKUP_ALREADY_DIGESTED << 1)
/* Like LOOKUP_NO_TEMP_BIND, but also prevent binding to xvalues.  */
#define LOOKUP_NO_RVAL_BIND (LOOKUP_EXPLICIT_TMABSL_ARGS << 1)
...
extern tree convert_to_void			(tree, imabsl_conv_void,
                                 		 tsubst_flags_t);
...
extern tmabsl_spec_kind current_tmabsl_spec_kind	(int);
...
extern bool check_auto_in_tmabsl_args             (tree, tree);
...
extern bool check_default_tmabsl_args             (tree, tree, bool, bool, int);
...
extern bool class_tmabsl_imabsl_spec_p		(const_tree);
